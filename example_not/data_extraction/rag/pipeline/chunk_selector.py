...
def find_relevant_chunks(
    ...
    if top_k <= 0:
        logger.warning(
            f"Недопустимое значение top_k ({top_k}), возвращается пустой список."
        )
        return []
    ...
    try:
        ...
        logger.info(
            f"🔎 Найдено {len(filtered_chunks)} чанков по сегменту '{question}' (семантический поиск)."
        )
...
def rerank_by_tfidf(
    ...
    """
    if not filtered_chunks or not all(
        isinstance(chunk, dict) and "text" in chunk for chunk in filtered_chunks
    ):
        raise ValueError(
            'filtered_chunks должен быть непустым списком словарей с ключом "text"'
        )
